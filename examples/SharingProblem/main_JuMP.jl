...
fig1 = plot(framestyle = :box, yticks = [1e0, 1e-2, 1e-4, 1e-6], size = (350,200), tickfont = 9, legendfont = font(9), guidefont = font(9), xlabel = "Number of iterations")
fig2 = plot(framestyle = :box, yticks = [1e0, 1e-1, 1e-2, 1e-3], size = (350,200), tickfont = 9, legendfont = font(9), guidefont = font(9), xlabel = "Number of iterations")

plot!(fig1, 1:length(traj_J), abs.(traj_J .- opt_cen) ./ abs(traj_J[1] - opt_cen), yscale = :log10, xlimit = [1, length(traj_J)], grid = true, label = "Optimality gap", linewidth=2)
...
plot!(fig2, 1:length(traj_res), traj_res, yscale = :log10, xlimit = [1, max(length(traj_res), length(traj_res_fADMM))], grid = true, label = "Primal-dual residual", linewidth=2, color=:red)
