...
function getGlobalCostFunction!(node::linknode, dict::Dict, J::Vector{NonlinearExpr}, opti)
    ...
    if node.children === nothing #Leaf
        sl = @variable(opti, [i=1:node.nV], base_name = string("sl",node.ID))
        @constraint(opti, sl .>= 1. ./vec_vars .- 1/p[1])
        @constraint(opti, sl .>= 0.)
        push!(J, sum(sl))
    elseif node.parent === nothing #Root
        push!(J, func(vec_vars; para = p))
    else
        η  = p[1]
        τ  = p[2]
        nc = p[3]
