...
node_iter  = Dict("fADMM" => Int64[], "hADMM" => Int64[])
tt_com     = Dict("fADMM" => Int64[], "hADMM" => Int64[])
max_com    = Dict("fADMM" => Int64[], "hADMM" => Int64[])
root_com   = Dict("fADMM" => Int64[], "hADMM" => Int64[])
obj_value  = Dict("fADMM" => Float64[], "hADMM" => Float64[])
opt_gap    = Dict("fADMM" => Float64[], "hADMM" => Float64[])
primal_res = Dict("fADMM" => Float64[], "hADMM" => Float64[])
dual_res   = Dict("fADMM" => Float64[], "hADMM" => Float64[])
...
function load_topology_csv(path::String)
    lines = readlines(path)
    length(lines) <= 1 && error("Topology file is empty: $path")

    rows = split.(lines[2:end], ",")
    nodes = Dict{String, linknode}()
    root = nothing

    for cols in rows
        node_id = String(cols[1])
        is_root = lowercase(cols[3]) == "true"
        nodes[node_id] = get!(nodes, node_id) do
            linknode(node_id)
        end
        if is_root
            root = nodes[node_id]
    ...
    root === nothing && error("No root node found in topology file: $path")

    for cols in rows
        node_id = String(cols[1])
        parent_id = String(cols[2])
        if !isempty(parent_id)
            parent = nodes[parent_id]
            child = nodes[node_id]
            set_relative!(parent, child)
        end
    end

    global countID = maximum(parse.(Int, collect(keys(nodes))))
...
for tp in 1:nTestTopo
    ...
    isfile(topo_csv_path) || error("Topology file not found: $topo_csv_path. Run main.jl first to generate and save the topologies.")
    root = load_topology_csv(topo_csv_path)
    println("Loaded topology from ", topo_csv_path)
