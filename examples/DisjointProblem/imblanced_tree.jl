...
function imbalanced_topo_gen!(node::linknode)
    global countID

    short_branch = linknode(string(countID += 1))
    long_branch = linknode(string(countID += 1))
    set_relative!(node, [short_branch, long_branch])

    remaining_nodes = nN - 3
    remaining_nodes > 0 || error("Imbalanced tree requires nN >= 4")
    long_branch_children = [linknode(string(countID += 1)) for _ in 1:remaining_nodes]
    set_relative!(long_branch, long_branch_children)
end
...
for tp in 1:nTestTopo
    ...
    imbalanced_topo_gen!(root)
    display(save_topology_shape(root, "Imbalanced"))
...
csv_filename = joinpath("data", "disjoint-problem", string("imblanced_tree-D=", nD, "-N=", nN, ".csv"))
